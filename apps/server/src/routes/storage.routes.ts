...
function hasAccess(ownerId: string, viewerId: string, sharing: { userId: string; level: AccessLevel }[], required: AccessLevel): boolean {
...
export function storageRoutes(auth: AuthService, engine: FsEngine) {
  ...
  router.patch(
    ...
    asyncRoute(async (req, res) => {
      ...
      const { title, parentId, order, icon, tags } = req.body as {
        title?: string;
